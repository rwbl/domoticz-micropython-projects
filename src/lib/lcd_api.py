...
class LcdApi:
    ...
    def clrrow(self, row):
        ...
        self.move_to(0, row)
        space = ' '
        # self.putstr(f'{space:20}')
        cols = str(self.num_columns)
        self.putstr(f'{space:<{cols}}')

    """NEW: rwbl"""
    def clrtext(self, col, row, cols):
        """Clear a text area by writing spaces.
        """
    ...
    """NEW: rwbl"""
    def putstrat(self, col, row, string):
        """Write the indicated string to the LCD at the col:row
        position and advances the cursor position appropriately.
        """
        self.move_to(col, row)
        self.putstr(string)
